...
if(isset($_COOKIE['login'])){
    ...
    $sql = "select `jiaoyi_id`,`jiaoyi_time`,`jiaoyi_address`,`jiaoyi_goods_id`,`jiaoyi_owner_phone`,`jiaoyi_status`,`jiaoyi_owner_status` from `jiaoyi_application` where `jiaoyi_faqiren_phone`='$phone' ORDER BY `jiaoyi_time` DESC ";
    if($result = $mysqli->query($sql)){
        ...
        for($i = 0;$i<$n;$i++){
            ...
            $info[$i]['owner']  = $row['jiaoyi_owner_phone'];
            ...
            if($result1 = $mysqli->query($sql1)) {
                ...
                if ($row1 != null) {
                    ...
                    $info[$i]['goodsimg2'] = $row1['goods_img2'];
                    $info[$i]['goodsimg3'] = $row1['goods_img3'];
                }
            }
        }
        $mysqli->close();
